...
class SkillGraphService:
    ...
    def list_runs(self) -> list[SkillGraphState]:
        """Return known local/demo skill runs without resuming graph execution."""

        runs: list[SkillGraphState] = []

        for run_id in sorted(self._run_ids):
            try:
                runs.append(self.get_run(run_id))
            except SkillRunNotFoundError:
                continue
