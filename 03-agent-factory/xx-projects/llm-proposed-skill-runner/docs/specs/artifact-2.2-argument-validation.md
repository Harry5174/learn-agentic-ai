...
Sprint E2.0 defines these contract concepts:
...
- `ValidatedSkillPlan`: validator output for accepted or rejected argument
  plans.
...
- Trusted default argument: `repository = "sandbox/demo-repo"`.
- Supported model-proposed argument: `repository`.
- V1 type: string.
- Status: optional because the dry-run tool has a safe default.
- Sensitive: false.

`draft_issue_comment`

- Supported model-proposed arguments: `issue_id`, `comment_body`.
- V1 types: integer and string.
- Status: required for meaningful dry-run drafts.
- Sensitive: false by default, with future support for sensitive comment-like
  fields if needed.

`trigger_workflow_dry_run`

- Supported model-proposed arguments: `workflow_name`, `ref`.
- V1 types: string and string.
- Status: required for high-risk workflow simulation.
- Sensitive: false.
...
## Sprint E2.3 Adversarial Suite

Sprint E2.3 adds adversarial tests for argument-level attacks:
...
The suite lives in `tests/test_adversarial_argument_validation.py`. The
documentation summary lives in `docs/adversarial-argument-validation.md`.
...
definition. Sprint E2.2 satisfies the execution-wiring slice. Sprint E2.3
satisfies the adversarial evidence and documentation slice.
